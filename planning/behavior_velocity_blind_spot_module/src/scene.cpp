  ...
  const int closest_idx, const geometry_msgs::msg::Pose & stop_line_pose) const
{
  ...
  if (!!areas_opt) {
    ...
    const auto & opposite_detection_areas = areas_opt.get().opposite_detection_areas;
    const auto & opposite_conflict_areas = areas_opt.get().opposite_conflict_areas;
    ...
    const auto adj =
      turn_direction_ == TurnDirection::LEFT
        ...
        : (turn_direction_ == TurnDirection::RIGHT ? (routing_graph_ptr->adjacentRight(lane))
                                                   : boost::none);
    const boost::optional<lanelet::ConstLanelet> opposite_adj =
